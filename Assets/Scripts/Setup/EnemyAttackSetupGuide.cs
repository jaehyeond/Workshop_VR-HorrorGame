...
2️⃣ Attack1 애니메이션에 Animation Event 추가:
...
public class EnemyAttackSetupWindow : EditorWindow
{
    ...
    void OnGUI()
    {
        ...
        if (GUILayout.Button("1️⃣ 모든 Enemy 프리팹에 EnemyAttackSystem 추가", GUILayout.Height(30)))
        {
            AutoSetupEnemyPrefabs();
        }
        
        if (GUILayout.Button("2️⃣ VR 플레이어에 VRPlayerHealth 추가", GUILayout.Height(30)))
        {
            AutoSetupPlayerHealth();
        }
        
        if (GUILayout.Button("3️⃣ 현재 설정 상태 검증", GUILayout.Height(30)))
        {
